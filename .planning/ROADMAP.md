...
**Depends on**: Phase 1
**Requirements**: [ANLY-01, ANLY-02, ANLY-03, ANLY-04, DSGN-01, DSGN-02, DSGN-03, DSGN-04, MOD-01, MOD-02, MOD-03, MOD-04, ARCH-01, ARCH-02, ARCH-03, ARCH-04, DATA-01, DATA-02, DATA-03, DATA-04, VERF-01, VERF-02, VERF-03, VERF-04, VERF-05, VERF-06, VERF-07, VERF-08]
**UI hint**: no
**Success Criteria** (what must be TRUE):
  ...
  5. 重构方案明确说明这不是代码搬运，而是基于原始行为进行架构模块重设计、接口重设计和代码重实现。
  6. 新架构满足高内聚低耦合，且恪守 KISS 原则：优先直接、清晰、易维护，避免过度工程化、过早抽象和不必要的防御性设计。
  7. `src/LoPS` 中实现了边界清晰的新模块，通过显式参数接收路径、配置和必要随机种子。
  8. `script` 中存在可运行新模块的入口脚本。
  9. 必要数据被整理到 `data` 并记录来源。
  10. 使用相同输入和随机种子，新旧实现输出完全一致；若完全一致不现实，必须记录原因、容差和差异结论。
  11. `src/LoPS/temp` 中的临时验证代码已清理。
  12. 完成记录说明运行方式、验证方式和一致性结论。
...
5. plan 阶段制定详细重构计划和实施计划，并在用户审核通过后进入 execute。计划必须体现架构模块重设计、接口重设计和代码重实现，而不是把旧代码搬运到新目录。
